...
> **걸음수는 프론트(기기 센서)에서 관리하고 DB에 저장하지 않는다.** 활동·위치 데이터를 서버에 축적하지 않기 위함(규칙 6, 프라이버시). 서버로 오는 건 **"적립 요청"뿐** — 걸음 원본이 아니라 그 결과 포인트만 원장에 남는다. 별도 `/steps/sync`·`/steps/today`는 두지 않는다(FE 로컬 상태 `expo-sensors`).
