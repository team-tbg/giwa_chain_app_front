# Naduri — API 스펙 (FE↔BE 계약 초안 v0.3)
...
> **FE 연동 메모**: 구글은 `expo-auth-session`(Google provider)으로 `id_token` 획득 → `/auth/social`. 필요한 값: `GOOGLE_CLIENT_ID`(Web/Android/iOS). 토큰은 `expo-secure-store` 에 저장. (2차 소셜은 미정.)
