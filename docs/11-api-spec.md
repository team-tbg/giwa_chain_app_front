# Naduri — API 스펙 (FE↔BE 계약 초안 v0.2)
...
>
> **v0.2 변경**: 로그인은 구글·토스만(이메일 제외) · `/auth/social`에 `referralCode`(가입 전 입력분) 추가 · `/cash/deposit`에 결제수단 enum, 입금 리스크 박스 제거 · 손실 고지는 변동 자산(BTC/금) 구매 시점으로(§8·§11·§13).
...
SNS 간편 로그인(구글·토스). 클라이언트가 provider 토큰을 획득 → 서버가 검증 후 자체 JWT 발급. **지갑(임베디드/AA)은 최초 로그인 시 서버가 자동 생성**(규칙 5).
...
```json
{
  "provider": "google",            // "google" | "toss"  (v10에서 이메일 로그인은 제외)
  "idToken": "<google id_token>",  // google: OIDC id_token
  "authCode": null,                 // toss: 인가코드(토스 SDK) — provider별 택1
...
> **FE 연동 메모**: 구글은 `expo-auth-session`(Google provider)으로 `id_token` 획득 → `/auth/social`. 토스는 토스 로그인 SDK 인가코드 → `authCode`. 필요한 값: `GOOGLE_CLIENT_ID`(iOS/Android/Web), 토스 클라이언트 키. 토큰은 `expo-secure-store` 에 저장.
...
### POST /steps/sync
기기 만보 센서값 업로드(멱등, 하루 누적 최대값 기준).
요청 `{ "date": "2026-07-22", "steps": 6432, "source": "expo-sensors" }` → `200 { "date", "steps", "goal" }`.

### GET /steps/today
`{ "steps": 6432, "goal": 10000, "claimableP": 64, "claimed": false }`.

### POST /steps/claim  `Idempotency-Key`
걸어서 받을 포인트를 적립(하루 1회, 일일 한도 반영).
응답 `200`:
```json
{ "grantedP": 64, "pointsAfter": 30218, "todayEarnedP": 104, "dailyCapP": 200, "cappedByDaily": false }
```
`422 DAILY_CAP_REACHED` — 한도 초과 시 `detail.remainingP`.
...
3. 토스 로그인 SDK 인가 방식(authCode vs idToken) 확정.
